...
compiles and runs programs on the emulated CPU. One C99 core does that in a terminal and, compiled to
WebAssembly, in your browser, where you can watch every head move, every state transition and every
tape cell change.

### [Open the live machine](https://jgoetzmann.github.io/Turing-Machine-OS/)

[Architecture](docs/architecture.md) · [Design decisions](docs/decisions.md) · [Status](docs/status.md) · [Turing machine](docs/turing-machine.md)
...
computer? The equivalence is provable, and it gets stated in a lecture more often than it gets built.
An 8080 for a head, a six-state kernel for the finite control, and on top of that a filesystem, four
compilers, a shell and eventually Pong, all of it living on one, two or four byte tapes and nothing
else.
...
checkers in `demos/tm/` measure exactly that. `pal1.tm` uses one tape and takes n²/2 + 3n/2 + 1 steps;
`pal2.tm` uses two and takes 3n + 3. At n = 64 that is 2,145 steps against 195, and underneath, on the
emulated 8080, 182,855 instructions against 31,353. Then the head-travel odometer reports that giving
each Turing-machine tape its own machine tape buys about 1%, because the interpreter's own instruction
fetches swamp the tape motion. The asymptotics hold and the implementation eats most of the win.
[`docs/turing-machine.md`](docs/turing-machine.md) is the accounting.

## What it does

The shell is written in tiny-C, the C subset this project compiles, and runs on the emulated 8080.
Typing `cc SHELL.C` inside the running OS reproduces the shipped 2,643-byte `shell.com` byte for byte,
which `cmp` asserts in
[`tests/integration/v2_ws6_06_self_compile.sh`](tests/integration/v2_ws6_06_self_compile.sh).
...
the editor in the browser. A Forth interpreter written in tiny-C runs inside the machine.
...
machine. `demos/fault` walks off the end on purpose, and its test asserts it faults at 32K and finishes
normally at 64K.
...
arrives, so the call completes exactly as if it had never waited. The machine is therefore a pure
function of its program, seed and input log.
([`src/kernel/kernel.c`](src/kernel/kernel.c), [`src/bios/bios.c`](src/bios/bios.c))

**Deterministic replay.** A seek restores the newest snapshot at or before the target and re-runs
instructions one at a time, re-applying logged inputs at the exact steps they arrived. Getting that
right meant deciding whether an input recorded at a snapshot's own step is inside or outside it,
suppressing output for steps the host has already seen while buffering output for steps being run for
the first time, refusing a seek whose program image has been recycled rather than silently replaying a
different binary, and restoring the caller's machine when a seek cannot land.
([`src/api/api.c`](src/api/api.c),
...
machine core. The native and WebAssembly builds differ by exactly one source file. Because a page
cannot block, frame pacing happens in the host loop, and the machine stays deterministic enough to
replay.
...
- Tests are named after the behavior they pin rather than the function they call:
  ...
  checklist with its audit verdicts left in: 53 items met, 35 partial with the gap written next to
  each, 1 not done.
...
- The Turing-machine compiler is checked three ways at once: the compiled program running on the
  ...
  pinned to the known values: Σ(2) = 4 ones in 6 steps, Σ(3) = 6 in 14, Σ(4) = 13 in 107.
- Disassembling the 2,643-byte shell image and reassembling the listing produces identical bytes. That
  only works because every undocumented alias byte has its own spelling.
...
implementation and the model part company.
...
`OUT 01H`, by design ([`docs/decisions.md`](docs/decisions.md) A7). The byte-for-byte shell rebuild is
a fixed point across two code paths.
...
`docs/decisions.md` B16 says why. Everything in `src/` is statically allocated, which sizes the
WebAssembly heap at build time, at the cost of one machine per process.

[`docs/status.md`](docs/status.md) has the full list of limits with the measured numbers.
...
and snapshots. Runs are scriptable and deterministic, which is how the shell tests drive the machine:
`printf 'dir\nhalt\n' | build/turingos --disk=build/disk/demo.img`.
...
[Design decisions](docs/decisions.md) is the five choices that shaped the machine, each with its
consequences and the alternatives that were rejected, and a one-line reference for everything else.
...
rather than from the implementation. The work was developed on a branch and squash-merged, which is why
one commit carries most of the code.

The machine was then audited adversarially against its own documentation, and the audit found real
defects: the 8080's auxiliary carry was computed as a half-borrow rather than as the carry out of bit 3
of the two's-complement add, so `SUB`, `SBB`, `CMP` and `DCR` all set it wrong and `ANA` used the
8085's rule; a seek to step 0 resurrected the anchor snapshot, which is the machine as it was before
any program was loaded. The fixes were audited in turn and the second pass found more.
`git log 96b054b..HEAD` is that trail, with the root cause in each commit body. Two fixes did not ship,
because every test I could write for them passed against the unfixed code, and a test that cannot fail

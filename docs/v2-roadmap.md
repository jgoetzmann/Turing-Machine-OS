# TuringOS v2 roadmap and acceptance criteria

**Status:** implemented and audited · **Plan date:** 2026-09-07 · **Baseline:** `main` @ `74a8714` (29/29 tests) · **Result:** `make test` 83/83, `make test-web` 63/63, `make test-e2e` 59/59, site live at <https://jgoetzmann.github.io/Turing-Machine-OS/>
...
Every checklist item below was re-verified against the repository in a read-only audit pass after the implementation landed, one pass per area. `[x]` = the auditor saw the behaviour or artefact; `[~]` = the substance exists but a stated sub-clause is not met, with the gap written next to the item; `[ ]` = not done. Counts after the audit-driven fixes: **55 done, 33 partial, 1 not done** of the 89 checklist items below. (GitHub issues per item were deliberately not created; the roadmap is the tracker.)
...
- **D8: honesty rule for the site.** Every claim on the site is backed by a test or by a named file in the repository; the architecture page's constants table is generated from source and diffed by a test, and every doc page links to its Markdown source at the built commit (WS0-02, WS8-07).
...
- [x] **WS0-09** `.github/workflows/ci.yml` runs `make test` on `ubuntu-latest` and `macos-latest`, plus one job with `-fsanitize=address,undefined`. Green on `main`; badge in README. *audit 2026-09-07:* 'Green on main' was not verifiable at audit time because the work was still on a development branch. *Resolved 2026-09-08:* the branch is merged and both workflows are green on `main`.
...
- [~] **WS4-04 Trace/age granularity**, *view lever*: age-decay window, trace on/off, native snapshot interval (`--snap=1000`). *audit 2026-09-07:* the age-decay window is not a lever (the panels derive it themselves). *Re-audited: the snapshot interval has both a `--snap=N` flag and a `snap=N` URL parameter, and `tests/kernel/test_v2_ws4_levers.c` now cites WS4-04.*
...
- [x] **WS4-06 Input mode**: blocking console (CONIN/READLINE) vs polled keys (`IN 0x03`); both always exist; the lever selects what the UI sends where; plus an *input script* (`--stdin-script=file`) for reproducible demos. *closed after re-audit: `--input=console|keys` is a CLI flag and `tests/kernel/test_v2_ws4_levers.c` cites WS4-06.*
...
- [x] **WS4-08 TM head-travel odometer**, *view lever/counter*: Σ |addr_i − addr_{i−1}| over every tape access (fetch + data), shown next to step count. It is the cost a physical single-head TM would pay for random access. Reset with the machine. Used by WS6-05. Test: a known 10-access sequence yields the expected sum.
- [~] **WS4-09** Every lever meets the "UI + URL + CLI + doc + test" rule; `docs/levers.md` has one table row per lever with range, default, and effect. *audit 2026-09-07:* the table has no separate 'effect' column and the age-decay window lever does not exist. *Re-audited: every lever now has a UI control, a URL parameter (`snap=N` was the last one missing), a CLI flag and a test; `tests/kernel/test_v2_ws4_levers.c` cites WS4-09.*
...
- [~] **WS6-01 First light**: the real programs from WS5-10 (`hello`, `count`, `echo`), each with a guided tour (WS6-10). *audit 2026-09-07:* One grouped page and one shared tour at /demos/hello covers all six programs; there is no per-program guided tour or /demos/count, /demos/echo page.
...
- [x] **WS8-01** Stack: Vite + TypeScript, static output in `web/dist/`; content pages authored as Markdown in `docs/` (the same files readable on GitHub, no copies) and rendered at build time. `.github/workflows/pages.yml` builds the wasm with the pinned emsdk, runs `npm ci && npm run build`, deploys with `actions/deploy-pages` on push to `main`. Live at `https://jgoetzmann.github.io/Turing-Machine-OS/`; `gh api repos/... -q .has_pages` is `true`. *audit 2026-09-07:* Site was not live at audit time: `has_pages=false` and `pages.yml` had not run, because the work was still on a development branch and `main` carried no workflows. *Resolved 2026-09-08:* `has_pages=true`, `pages.yml` runs on every push to `main`, and the site is live at the URL above.

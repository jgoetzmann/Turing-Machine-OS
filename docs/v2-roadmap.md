...
- [~] **WS4-04 Trace/age granularity**, *view lever*: age-decay window, trace on/off, native snapshot interval (`--snap=1000`). *audit 2026-09-07:* Age-decay window is not a lever (panels auto-derive it); the snapshot interval has a `--snap=N` CLI flag but no URL parameter. *Re-audited: `tests/kernel/test_v2_ws4_levers.c` now cites WS4-04.*
...
- [~] **WS4-09** Every lever meets the "UI + URL + CLI + doc + test" rule; `docs/levers.md` has one table row per lever with range, default, and effect. *audit 2026-09-07:* the snapshot interval has no URL parameter and the table has no separate 'effect' column; the age-decay window lever does not exist. *Re-audited: every lever has a CLI flag (`--input=`, `--snap=`) and `tests/kernel/test_v2_ws4_levers.c` cites WS4-09.*

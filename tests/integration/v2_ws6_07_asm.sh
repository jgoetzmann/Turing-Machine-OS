...
./build/disasm build/tests/hello_asm.com | sed -E 's/^[0-9A-F]{4}: ([0-9A-F]{2} ?){1,3} +//' | sed 's/\*//' > build/tests/hello_re.asm
...
./build/disasm build/bin/shell.com | sed -E 's/^[0-9A-F]{4}: ([0-9A-F]{2} ?){1,3} +//' | sed 's/\*//' > build/tests/shell_re.asm
